...
import { FaBars, FaFileAlt, FaBell, FaHome } from "react-icons/fa";
import { Link } from "react-router-dom";

const TenderOwnerSidebar = () => {
  ...
  return (
    <div className={`h-screen sticky left-0 top-0 ${isOpen ? "w-64" : "w-20"}`}>
      <nav className="bg-gray-900 text-white flex justify-between items-center px-6 py-2 shadow h-16">
        <Link to="/dashboard" className="flex items-center space-x-5 rtl:space-x-reverse">
...
export default TenderOwnerSidebar;
